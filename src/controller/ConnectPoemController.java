...
public class ConnectPoemController extends MouseAdapter {
	...
	public ConnectPoemController(Model model, Application app) {
		this.model = model;
		this.app = app;
	...
	protected boolean release(int x, int y) {
		...
		ArrayList<Poem> poems = model.getBoard().poems;
		Row firstRowofSelectedPoem = selectedPoem.getRowList().get(0);
		Row lastRowofSelectedPoem = selectedPoem.getRowList().get(
				selectedPoem.getRowList().size() - 1);

		for (int i = 0; i < poems.size(); i++) {
			Poem poem = poems.get(i);
			Row firstRowofPoem = poem.getRowList().get(0);
			Row lastRowofPoem = poem.getRowList().get(
					poem.getRowList().size() - 1);
			int distanceToTop = firstRowofPoem.getY()
					- (lastRowofSelectedPoem.getY() + lastRowofSelectedPoem
							.getHeight());
			int distanceToBottom = firstRowofSelectedPoem.getY()
					- (lastRowofPoem.getY() + lastRowofPoem.getHeight());
			if (distanceToTop > 0 && distanceToTop < 20) {
				// if selectedPoem is at the top of Poem
				if ((firstRowofPoem.getX() - lastRowofSelectedPoem.getWidth()) < lastRowofSelectedPoem
						.getX()
						&& lastRowofSelectedPoem.getX() < (firstRowofPoem
								.getX() + firstRowofPoem.getWidth())) {
					selectedPoem.setLocation(firstRowofSelectedPoem.getX(),
							poem.getY() - selectedPoem.getHeight());
					poem.addPoem(selectedPoem, false);
					model.getBoard().removePoem(selectedPoem);
				}
			} else if (distanceToBottom > 0 && distanceToBottom < 20) {
				// if selectedPoem is at the bottom of Poem
				if ((lastRowofPoem.getX() - firstRowofSelectedPoem.getWidth()) < firstRowofSelectedPoem
						.getX()
						&& firstRowofSelectedPoem.getX() < (lastRowofPoem
								.getX() + lastRowofPoem.getWidth())) {
					selectedPoem.setLocation(firstRowofSelectedPoem.getX(),
							lastRowofPoem.getY() + lastRowofPoem.getHeight());
					poem.addPoem(selectedPoem, true);
					model.getBoard().removePoem(selectedPoem);
				}
			}
		}
