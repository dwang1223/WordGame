...
/**
 * TestDisconnectRowController Class
 * 
 * @author diwang
 *
 */
public class TestDisconnectRowController extends TestCase {
	public void testDisconnectRowFromPoemOf2Rows() {
		...
		DisconnectRowController disconnectRowController = new DisconnectRowController(
				model, app);
		disconnectRowController.register();
		disconnectRowController.select(105, 105);
		disconnectRowController.drag(1000, 1000);
		disconnectRowController.drag(200, 150);
		disconnectRowController.drag(200, 200);
		disconnectRowController.release(200, 200);
		disconnectRowController.select(210, 160);
	}

	public void testDisconnectRowFromPoem() {
		Board curBoard = new Board();
		Word word1 = new Word(100, 100, "di", "noun", true, false, false, false);
		Word word2 = new Word(120, 100, "wang", "noun", true, false, false,
				false);
		...
		DisconnectRowController disconnectRowController = new DisconnectRowController(
				model, app);
		disconnectRowController.register();
		disconnectRowController.select(110, 130);
		disconnectRowController.select(10, 10);
	}
}
